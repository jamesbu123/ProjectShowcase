...
for dmg in damages:
  if dmg.find('M') != -1:
    updated_damage.append(float(dmg[0:dmg.find('M')])*conversion["M"])
  if dmg.find("B") != -1:
    updated_damage.append(float(dmg[0:dmg.find("B")])*conversion["B"])
  if dmg == "Damages not recorded":
    updated_damage.append(dmg)
...
# Create and view the hurricanes dictionary
  ...
  return new_table
hurricanes = create_dictionary(names, months, years, max_sustained_winds, areas_affected, updated_damage, deaths)

# 3
# Organizing by Year
def organize_list(hurricanes):
  hurricanes_by_year={}
...
hurricanes_by_year = organize_list(hurricanes)
...
def highest_death(hurricanes):
  max_mortality_cane = 'Cuba'
  max_mortality = 0
  for cane in hurricanes:
...
max_mortality_cane, max_mortality = highest_death(hurricanes)
